...
.nav-dropdown-menu {
  ...
  border-radius: 14px; box-shadow: 0 12px 32px rgba(44,62,107,.10);
  z-index: 999; padding: 6px; animation: dropIn .18s ease;
}
...
.nav-sub-dropdown-menu {
  ...
  border-radius: 14px; box-shadow: 0 12px 32px rgba(44,62,107,.10);
  z-index: 1000; padding: 6px; animation: dropIn .18s ease;
}
@media (max-width: 1100px) { .nav-sub-dropdown-menu { left: auto; right: calc(100% + 4px); } }
.nav-sub-dropdown:hover .nav-sub-dropdown-menu,
.nav-sub-dropdown:focus-within .nav-sub-dropdown-menu { display: block; }
.dd-item {
  display: flex; align-items: center; gap: 8px; padding: 9px 13px;
  border-radius: 10px; font-size: .84rem; font-weight: 500; color: #374151;
  ...
  <div class="max-w-7xl mx-auto px-4">
    <div class="flex items-center justify-between h-16 md:h-20">
      ...
      <div class="hidden md:flex items-center gap-1 text-sm font-medium text-gray-700">
        ...
        <a href="<?= BASE_URL ?>/" class="nav-link px-3 py-2 rounded-lg hover:bg-cream hover:text-sage transition <?= $current_slug === '' ? 'active' : '' ?>">Home</a>
        <a href="<?= BASE_URL ?>/#tentang" class="nav-link px-3 py-2 rounded-lg hover:bg-cream hover:text-sage transition">Tentang</a>
        <a href="<?= BASE_URL ?>/#layanan" class="nav-link px-3 py-2 rounded-lg hover:bg-cream hover:text-sage transition">Layanan</a>
            ...
            ?>
              ...
              <div class="nav-sub-dropdown">
                <div class="dd-item has-sub">
                  <a href="<?= BASE_URL ?>/<?= e($node['slug']) ?>/" class="flex-1 min-w-0 <?= $current_slug === $node['slug'] ? 'text-sage font-semibold' : '' ?>"><?= e($node['name']) ?></a>
                  <svg class="chevron-right w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                </div>
                <div class="nav-sub-dropdown-menu">
                  <?php foreach ($node['children'] as $ch): ?>
                  <a href="<?= BASE_URL ?>/<?= e($ch['slug']) ?>/" class="dd-item <?= $current_slug === $ch['slug'] ? 'text-sage font-semibold' : '' ?>"><?= e($ch['name']) ?></a>
                  <?php endforeach; ?>
                </div>
              </div>
              <?php else: ?>
              <a href="<?= BASE_URL ?>/<?= e($node['slug']) ?>/" class="dd-item <?= $current_slug === $node['slug'] ? 'text-sage font-semibold' : '' ?>"><?= e($node['name']) ?></a>
              <?php endif; ?>
            <?php endforeach; ?>
          </div>
        </div>

        <a href="<?= BASE_URL ?>/#area"      class="nav-link px-3 py-2 rounded-lg hover:bg-cream hover:text-sage transition">Area Kirim</a>
        <a href="<?= BASE_URL ?>/#testimoni" class="nav-link px-3 py-2 rounded-lg hover:bg-cream hover:text-sage transition">Testimoni</a>
        <a href="<?= BASE_URL ?>/#faq"       class="nav-link px-3 py-2 rounded-lg hover:bg-cream hover:text-sage transition">FAQ</a>

        <!-- ✅ MENU BLOG — DITAMBAHKAN DI SINI -->
        <a href="<?= BASE_URL ?>/blog/"
           class="nav-link px-3 py-2 rounded-lg hover:bg-cream hover:text-sage transition
                  <?= $current_slug === 'blog' || str_starts_with($current_slug, 'blog/') ? 'active' : '' ?>">
          Blog
        </a>
    ...
    <div id="mobile-menu" class="md:hidden hidden pb-4 border-t border-cream-dark mt-2">
      <div class="flex flex-col gap-0.5 pt-2">
        ...
        <a href="<?= BASE_URL ?>/" class="px-4 py-2.5 rounded-xl hover:bg-cream text-sm font-medium text-gray-700 hover:text-sage transition">🏠 Home</a>
        <a href="<?= BASE_URL ?>/#tentang" class="px-4 py-2.5 rounded-xl hover:bg-cream text-sm font-medium text-gray-700 hover:text-sage transition mob-close">💬 Tentang</a>
        <a href="<?= BASE_URL ?>/#layanan" class="px-4 py-2.5 rounded-xl hover:bg-cream text-sm font-medium text-gray-700 hover:text-sage transition mob-close">🌸 Layanan</a>

        <!-- Produk accordion -->
        <div>
          <button class="mob-acc-btn w-full flex items-center justify-between px-4 py-2.5 rounded-xl hover:bg-cream text-sm font-medium text-gray-700 hover:text-sage transition" onclick="toggleMobAcc(this)">
            <span>🛍️ Produk</span>
            <svg class="acc-arrow w-4 h-4 text-sage opacity-60" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
          </button>
          <div class="mob-acc-content pl-4">
            <?php $mob_tree = $desktop_tree; foreach ($mob_tree as $node): $has_kids = !empty($node['children']); ?>
              <?php if ($has_kids): ?>
              <div class="mt-0.5">
                <button class="mob-acc-btn w-full flex items-center justify-between px-3 py-2 rounded-lg hover:bg-sage/10 text-sm font-medium text-gray-700 hover:text-sage transition" onclick="toggleMobAcc(this)">
                  <span><?= e($node['name']) ?></span>
                  <svg class="acc-arrow w-3.5 h-3.5 text-sage opacity-60" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                </button>
                <div class="mob-acc-content pl-3 border-l-2 border-sage/20 ml-3">
                  <a href="<?= BASE_URL ?>/<?= e($node['slug']) ?>/" class="block px-3 py-1.5 text-xs font-semibold text-sage hover:underline mob-close">Lihat semua <?= e($node['name']) ?> →</a>
                  <?php foreach ($node['children'] as $ch): ?>
                  <a href="<?= BASE_URL ?>/<?= e($ch['slug']) ?>/" class="block px-3 py-1.5 text-sm text-gray-600 hover:text-sage transition mob-close"><?= e($ch['name']) ?></a>
                  <?php endforeach; ?>
                </div>
              </div>
              <?php else: ?>
              <a href="<?= BASE_URL ?>/<?= e($node['slug']) ?>/" class="block px-3 py-2 mt-0.5 rounded-lg hover:bg-sage/10 text-sm font-medium text-gray-700 hover:text-sage transition mob-close"><?= e($node['name']) ?></a>
              <?php endif; ?>
            <?php endforeach; ?>
          </div>
        </div>

        <a href="<?= BASE_URL ?>/#area"      class="px-4 py-2.5 rounded-xl hover:bg-cream text-sm font-medium text-gray-700 hover:text-sage transition mob-close">📍 Area Kirim</a>
        <a href="<?= BASE_URL ?>/#testimoni" class="px-4 py-2.5 rounded-xl hover:bg-cream text-sm font-medium text-gray-700 hover:text-sage transition mob-close">⭐ Testimoni</a>
        <a href="<?= BASE_URL ?>/#faq"       class="px-4 py-2.5 rounded-xl hover:bg-cream text-sm font-medium text-gray-700 hover:text-sage transition mob-close">❓ FAQ</a>

        <!-- ✅ MENU BLOG MOBILE — DITAMBAHKAN DI SINI -->
        <a href="<?= BASE_URL ?>/blog/"
           class="px-4 py-2.5 rounded-xl hover:bg-cream text-sm font-medium hover:text-sage transition mob-close
                  <?= str_starts_with($current_slug, 'blog') ? 'text-sage font-semibold' : 'text-gray-700' ?>">
          📝 Blog
        </a>
