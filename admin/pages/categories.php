...
<div class="grid lg:grid-cols-3 gap-6">
  ...
  <div class="lg:col-span-1">
    <div class="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm sticky top-20">
      ...
      <form method="POST" enctype="multipart/form-data" class="space-y-4">
        ...
        <div><label class="form-label">Nama Kategori *</label><input type="text" name="name" required class="form-input" value="<?= e($edit['name'] ?? '') ?>"></div>
        <div>
          <label class="form-label">Slug (URL) *</label>
          <input type="text" name="slug" required class="form-input" value="<?= e($edit['slug'] ?? '') ?>" placeholder="karangan-bunga-papan-jakarta-utara">
        ...
        <div><label class="form-label">Konten Halaman (SEO)</label><textarea name="content" rows="5" class="form-textarea" placeholder="Konten HTML untuk halaman kategori..."><?= htmlspecialchars($edit['content'] ?? '') ?></textarea></div>
        <div>
          <label class="form-label">Gambar</label>
          <?php if (!empty($edit['image'])): ?><img src="<?= e(imgUrl($edit['image'], 'category')) ?>" class="w-20 h-20 object-cover rounded-lg mb-2 border"><?php endif; ?>
        ...
        <div class="flex gap-3 pt-2">
          ...
          <?php if ($edit): ?><a href="?page=categories" class="px-4 py-2.5 bg-gray-100 text-gray-600 rounded-xl text-sm font-medium">Batal</a><?php endif; ?>
        </div>
      </form>
    </div>
  </div>
  ...
  <div class="lg:col-span-2">
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      <div class="px-6 py-4 border-b border-gray-100"><h2 class="font-serif font-semibold text-navy text-base">Daftar Kategori</h2></div>
      <table class="w-full data-table">
        <thead><tr><th>Nama</th><th>Slug</th><th>Status</th><th>Aksi</th></tr></thead>
        <tbody>
          <?php foreach ($rows as $r): ?>
          <tr>
            <td class="font-medium text-sm"><?= e($r['name']) ?></td>
            ...
            <td>
              <div class="flex gap-2">
                <a href="?page=categories&action=edit&id=<?= $r['id'] ?>" class="text-xs bg-blue-50 text-blue-600 hover:bg-blue-100 px-3 py-1 rounded-lg font-medium transition">Edit</a>
                <a href="?page=categories&action=delete&id=<?= $r['id'] ?>" onclick="return confirm('Hapus kategori ini?')" class="text-xs bg-red-50 text-red-600 hover:bg-red-100 px-3 py-1 rounded-lg font-medium transition">Hapus</a>
